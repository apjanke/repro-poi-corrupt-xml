classdef Log4jConfigurator
    ...
    % Examples:
    %
    % blah.logger.Log4jConfigurator.configureBasicConsoleLogging
    %
    % blah.logger.Log4jConfigurator.setLevels({'root','DEBUG'});
    %
    % blah.logger.Log4jConfigurator.setLevels({
    %     'root'    'INFO'
    %     'net.apjanke.logger.swing'  'DEBUG'
    %     });
    %
    % blah.logger.Log4jConfigurator.prettyPrintLogConfiguration
    %
    % % Display fully-qualified class/category names in the log output:
    % blah.logger.Log4jConfigurator.setRootAppenderPattern(...
    %    ['%d{HH:mm:ss.SSS} %p %c - %m' sprintf('\n')]);
    %
    % % Bring up the configuration GUI
    % blah.logger.Log4jConfigurator.showGui
    ...
    methods (Static)
        ...
        function setLevels(levels)
            ...
            %
            % blah.logger.Log4jConfigurator.setLevels(levels)
            %
            ...
            for i = 1:size(levels, 1)
                ...
                level = blah.logger.Log4jConfigurator.getLog4jLevel(levelName);

classdef Dispstr
  ...
  methods (Static)
    ...
    function disp(x)
      if iscell(x)
        blah.logger.internal.Dispstr.dispCell(x);
      elseif isnumeric(x)
        builtin('disp', x);
      elseif isstruct(x)
        blah.logger.internal.Dispstr.dispStruct(x);
      elseif istable(x)
        blah.logger.internal.Dispstr.prettyprintTabular(x);
      else
        builtin('disp', x);
      end
    end
    ...
    function display(x) %#ok<DISPLAY>
      ...
      blah.logger.internal.Dispstr.disp(x);
    end
    ...
    function dispCell(c)
      ...
      fprintf('%s\n', blah.logger.internal.Dispstr.prettyprintArray(dstrs));
    end
    ...
    function out = mycombvec(vecs)
      ...
      switch numel(vecs)
        ...
        otherwise
          a = vecs{1}(:);
          rest = vecs(2:end);
          rest_combs = blah.logger.internal.Dispstr.mycombvec(rest);
    ...
    function out = prettyprintArray(strs)
      ...
      if ismatrix(strs)
        out = blah.logger.internal.Dispstr.prettyprintMatrix(strs);
      else
        sz = size(strs);
        high_sz = sz(3:end);
        high_ixs = {};
        ...
        page_ixs = blah.logger.internal.Dispstr.mycombvec(high_ixs);
        chunks = {};
        for i_page = 1:size(page_ixs, 1)
          page_ix = page_ixs(i_page,:);
          chunks{end+1} = sprintf('(:,:,%s) = ', ...
            strjoin(blah.logger.internal.Dispstr.num2cellstr(page_ix), ',')); %#ok<*AGROW>
          page_ix_cell = num2cell(page_ix);
          page_strs = strs(:,:,page_ix_cell{:});
          chunks{end+1} = blah.logger.internal.Dispstr.prettyprintMatrix(page_strs);
        end
        out = strjoin(chunks, '\n');
      end
      if nargout == 0
    ...
    function out = prettyprintMatrix(strs)
      ...
      widths = max(lens, 1);
      formats = blah.logger.internal.Dispstr.sprintfv('%%%ds', widths);
    ...
    function out = prettyprintStruct(s)
      ...
      if isscalar(s)
        ...
        out = blah.logger.internal.Dispstr.dispc(s);
      end
    ...
    function out = prettyprintTabular(t)
      ...
      if nVars == 0
        out = sprintf('%s table with zero variables', blah.logger.internal.Dispstr.size2str(size(t)));
        return;
      end
      ...
      out = blah.logger.internal.Dispstr.prettyprintTabular_generic(varNames, varVals, true);      
      if nargout == 0
        fprintf('%s\n', out);
      end
    end

classdef Logger
  ...
  % arguments.
  %    blah.logger.info(format, varargin)
  ...
  % message.
  %    blah.logger.warn(exception, format, varargin)
  %
  % See also:
  % blah.logger.error
  % blah.logger.warn
  ...
  % Examples:
  %
  % log = blah.logger.Logger.getLogger('foo.bar.FooBar');
  ...
  methods (Static)
    function out = getLogger(identifier)
      ...
      out = blah.logger.Logger(jLogger);
    end
  end

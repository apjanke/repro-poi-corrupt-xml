...
% that's done in blah.Settings.discover. It has to be that way so the package
% settings state can handle a `clear classes` gracefully.

% Do not re-initialize if already initialized
if blah.internal.utils.getpackageappdata('initialized')
  return
end

% Don't depend on globals, to avoid circular dependency
%distroot = blah.globals.distroot;
...
blah.internal.utils.setpackageappdata('initialized', true);

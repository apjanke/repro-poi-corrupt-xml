...
reporoot = blah.globals.distroot;
origDir = pwd;
RAII.cd = onCleanup(@() cd(origDir));
cd(reporoot)

fprintf('Building repro-poi-corrupt-xml...\n')

cfgFile = fullfile(reporoot, '.mlproject.json');
config = jsondecode(blah.internal.util.readtext(cfgFile));
...
fprintf('repro-poi-corrupt-xml built.\n')
